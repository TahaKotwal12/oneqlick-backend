...
async def unified_search(
    ...
    search_type: str = Query("all", description="Search type: all, restaurants, dishes, categories"),
    is_veg_only: Optional[bool] = Query(None, description="Filter for vegetarian items only"),
    is_open: Optional[bool] = Query(None, description="Filter for currently open restaurants"),
    sort_by: str = Query("relevance", description="Sort by: relevance, distance, rating, price_low, price_high"),
    db: Session = Depends(get_db),
    current_user: Optional[User] = Depends(get_optional_current_user)
):
    """
    Unified search endpoint that searches restaurants, dishes, and categories.
    
    This endpoint provides a comprehensive search experience by searching across:
    - Restaurant names, descriptions, and cuisine types
    - Food item names, descriptions, and ingredients
    - Category names and descriptions
    
    Results are filtered by location and sorted by relevance or other criteria.
    """
    try:
        logger.info(f"Unified search for query: '{query}' at location: ({latitude}, {longitude})")
        
        # Validate search_type parameter
        valid_search_types = ['all', 'restaurants', 'dishes', 'categories']
        if search_type not in valid_search_types:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid search_type value. Must be one of: {', '.join(valid_search_types)}"
            )
        
        # Validate sort_by parameter
        valid_sorts = ['relevance', 'distance', 'rating', 'price_low', 'price_high']
        if sort_by not in valid_sorts:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid sort_by value. Must be one of: {', '.join(valid_sorts)}"
        ...
        # Prepare search term for ILIKE queries
        ...
        if search_type in ['all', 'restaurants']:
            ...
            # Apply filters
            if is_veg_only is not None:
                restaurant_query = restaurant_query.filter(Restaurant.is_pure_veg == is_veg_only)
                ...
                if distance <= radius_km:
                    ...
                    if is_open is None or is_open == currently_open:
                        restaurant_results.append({
                            ...
                            'is_pure_veg': restaurant.is_pure_veg,
                            'distance': distance,
                            'relevance_score': _calculate_restaurant_relevance(restaurant, query)
                        })
        
        # Search food items if requested
        if search_type in ['all', 'dishes']:
            # Get nearby restaurants first for location filtering
            nearby_restaurant_ids = []
            if restaurant_results:
                nearby_restaurant_ids = [r['id'] for r in restaurant_results]
            else:
                # If no restaurants found, get nearby restaurants for dish search
                all_restaurants = db.query(Restaurant).filter(Restaurant.status == 'active').all()
                for restaurant in all_restaurants:
                    distance = calculate_distance(
                        latitude,
                        longitude,
                    ...
                    if distance <= radius_km:
                        nearby_restaurant_ids.append(restaurant.restaurant_id)
            
            if nearby_restaurant_ids:
                # Build a map of nearby restaurants for quick lookup
                nearby_restaurants = db.query(Restaurant).filter(
                    Restaurant.restaurant_id.in_(nearby_restaurant_ids)
                ).all()
                restaurant_map = {r.restaurant_id: r for r in nearby_restaurants}

                dish_query = db.query(FoodItem).filter(
                    FoodItem.restaurant_id.in_(nearby_restaurant_ids),
                    FoodItem.status == 'available',
                    (FoodItem.name.ilike(search_term)) |
                    (FoodItem.description.ilike(search_term)) |
                    (FoodItem.ingredients.ilike(search_term))
                )
                
                # Apply filters
                if is_veg_only is not None:
                    dish_query = dish_query.filter(FoodItem.is_veg == is_veg_only)
                ...
                for dish in dishes:
                    ...
                    dish_results.append({
                        ...
                        'restaurant_delivery_time': restaurant.avg_delivery_time,
                        'restaurant_delivery_fee': float(restaurant.delivery_fee or 0),
                        'restaurant_min_order': float(restaurant.min_order_amount or 0),
                        'restaurant_is_open': is_restaurant_currently_open(restaurant),
                        'restaurant_is_veg': restaurant.is_veg,
                        'restaurant_is_pure_veg': restaurant.is_pure_veg,
                        'distance': distance,
                        'relevance_score': _calculate_dish_relevance(dish, query)
                    })
        ...
        if search_type in ['all', 'categories']:
            ...
            for category in categories:
                category_results.append({
                    ...
                    'relevance_score': _calculate_category_relevance(category, query)
                })
        
        # Combine and sort results
        all_results = restaurant_results + dish_results + category_results
...
@router.get("/recent", response_model=CommonResponse[NearbyRestaurantsResponse])
async def get_recent_restaurants(
    ...
    Returns a list of restaurants sorted by creation date in descending order.
    """
    try:
        logger.info(f"Fetching recent restaurants, limit: {limit}, offset: {offset}")
        
        # Query restaurants sorted by creation date
        query = db.query(Restaurant).filter(
            Restaurant.status == 'active'
        ...
        for restaurant in restaurants:
            ...
            restaurant_dict = {
                ...
                'opening_time': restaurant.opening_time,
                'closing_time': restaurant.closing_time,
                'distance': 0.0,  # Distance not relevant for recent list, or could be calculated if lat/long provided
                'location': location,
                'offers': offer_responses
            }
        ...
        return CommonResponse(
            code=200,
            message="Recent restaurants retrieved successfully",
            message_id="RECENT_RESTAURANTS_SUCCESS",
            data=NearbyRestaurantsResponse(
                restaurants=restaurant_responses,
                total_count=total_count,
                has_more=has_more
        ...
        logger.error(f"Error fetching recent restaurants: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch recent restaurants: {str(e)}"
        )
